...
def write_source(file_name, original_files, source):
    header = """\
# -*- coding: utf-8 -*-
...
def formula_to_python_source(node):
    ...
    symbols = []
    find_node(node['expression'], out=symbols, type='symbol')
...
def chap_to_python_source(application, json_file_name):
    regle_nodes = list(filter(
        lambda node: application in node['applications'],
        read_ast_json_file(os.path.join(args.json_dir, json_file_name)),
        ))
...
def main():
    ...
    write_source(
        ...
        original_files='tgvH.json and tgvH.m',
        source='variable_definition_by_name = ' + value_to_python_source(variable_definition_by_name),
        )

    for index in range(1, 4):
        write_source(
            file_name='chap_{}.py'.format(index),
            original_files='chap-{index}.json and chap-{index}.m'.format(index=index),
            source=chap_to_python_source(application=args.application, json_file_name='chap-{}.json'.format(index)),
            )
